    ...
    <div class="min-vh-100 container my-2">
        <div class="d-flex align-items-center mb-2">
            ...
            @if (Auth::user()->role == 1 && $request->status < 2)
                <a class="btn btn-custom-primary d-flex align-items-center shadow-sm" href="/air-offer/add/{{ $request->id }}"><i class="bi bi-plus me-1"></i>Add New Offer</a>
            @endif
        </div>
                ...
                <table class="table-striped table">
                    <thead class="table-dark">
                        <tr>
                            ...
                            <th scope="col" colspan="2">Date of Destination</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($offers as $offer)
                            <tr>
                                ...
                                <td style="vertical-align: middle;">{{ $offer->date_of_departure }}</td>
                                <td style="vertical-align: middle;">{{ $offer->date_of_destination }}</td>

                                <td style="vertical-align: middle;" scope="row">
                                    @if ($request->status == 1)
                                        <form action="/air-offer/choose/{{ $offer->airRequest->id }}/{{ $offer->id }}" method="post">
                                            @csrf
